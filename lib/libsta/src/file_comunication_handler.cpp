...
std::vector<char> receive_vector(int sockfd, std::string output_filename, unsigned long compressedSize)
{
    ...
    if (read(sockfd, &dataSize, sizeof(dataSize)) < 0) {  // Read the data size
    ...
    while (file_data.size() < dataSize && (bytesRead = read(sockfd, buffer, BUFFER_SIZE)) > 0) { // Read the data
...
void decompressFile(std::vector<char> file_data, std::string output_filename){
    // Decompress data
    std::vector<char> decompressedData(file_data.size() * 10); // Guess the decompressed size as 10 times the compressed size
...
void send_vector(int sockfd, std::vector<char> file_data, unsigned long compressedSize){
    ...
    if (write(sockfd, file_data.data(), compressedSize) < 0){ // Send the data
